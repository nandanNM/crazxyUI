...
const Navbar2 = () => {
  ...
  return (
    <nav className="w-full shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <ul className="hidden md:flex gap-8 text-sm font-medium">
          {links2.map(link => (
            <li
              key={link.id}
              className="cursor-pointer hover:text-yellow-400 after:transition-all duration-300 ease-in-out"
            >
              {link.name}
            </li>
          ))}
        ...
        <div className="hidden md:block">
          <button className="text-sm font-semibold px-3 py-2 rounded-full bg-yellow-500 hover:bg-yellow-400 transition cursor-pointer">
            Get Started
          </button>
        </div>
      ...
      {/* Mobile Sidebar */}
      {open && (
        <div className="md:hidden w-full  px-4 pb-4 shadow-md">
          <ul className="flex flex-col gap-4 mt-2">
            {links2.map(link => (
              <li
                key={link.id}
                className="text-sm font-medium cursor-pointer hover:text-yellow-300"
              >
                {link.name}
              </li>
            ))}
          </ul>
          <button className="mt-4 w-fit bg-white text-blue-600 px-3 py-1 rounded-full font-medium text-sm hover:bg-yellow-400 hover:text-black transition cursor-pointer">
            Get Started
          </button>
        </div>
      )}
